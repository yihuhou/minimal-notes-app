...
- `minimal-notes-health-history.json` 是更原始的健康历史文件；应用会在没有 `minimal-notes-health-calendar.json` 时尝试读取它。
...
- `minimal-notes-health-history.json` is a more raw health history file. The app tries it when `minimal-notes-health-calendar.json` is absent.
